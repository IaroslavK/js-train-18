...
function decodeURIComponentWrapper(encodedString) {
  try {
    const urlStr = decodeURI(encodedString);
    return urlStr;
  } catch (err) {
    const uriError = new URIError("Помилка декодування URI");
    return uriError;
  }
...
function findEvenNumber(numbers) {
  try {
    let evenNumber;
    evenNumber = numbers.find((elem) => elem % 2 === 0);
    console.log(numbers);
    if (!evenNumber) {
...
function validateUser(user) {
  try {
    if (typeof user !== "object") {
      throw new TypeError("Об'єкт користувача не вказано!");
    }
...
function evaluateExpression(expression) {
  try {
    ...
    const evalErr = new EvalError(err.message);
    return evalErr;
  }
